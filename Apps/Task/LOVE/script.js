...
function renderPartnerCard(elementId, partner) {
    ...
    card.innerHTML = `
                ...
                <div class="partner-name">
                    <h3 class="edit-feeling-btn" data-partner-id="${partner.userId}" >${partner.name}</h3>
                </div>
...
function updateChatModeVisuals(isSecret) {
    // Cập nhật class cho phần header
    chatHeader.classList.remove('secret-mode', 'normal-mode');
    chatHeader.classList.add(isSecret ? 'secret-mode' : 'normal-mode');

    // Cập nhật class cho nút bấm
    toggleChatModeBtn.classList.remove('secret-mode', 'normal-mode');
    toggleChatModeBtn.classList.add(isSecret ? 'secret-mode' : 'normal-mode');

    // Cập nhật nội dung nút
    toggleChatModeBtn.innerHTML = isSecret
        ? '<i class="fas fa-user-secret"></i>Bí mật'
        : '<i class="fas fa-comments"></i>Bình thường';
}
