...
const firebaseConfig = {
    apiKey: "AIzaSyDY6AdSizuLx4H1f_AocrB5Gh0xpB-rSkk",
    authDomain: "so-cham-cong.firebaseapp.com",
    projectId: "so-cham-cong",
    storageBucket: "so-cham-cong.firebasestorage.app",
    messagingSenderId: "863996288784",
    appId: "1:863996288784:web:8de285a4e59a4e8ad50f95",
    measurementId: "G-41LDPJWZ6L"
};
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const init = () => {
        // Hàm này bây giờ chỉ có nhiệm vụ duy nhất là vẽ giao diện
        // Nó được gọi SAU KHI loadAllData đã chạy xong.
        let today = new Date();
    ...
    // THAY THẾ KHỐI MÃ NÀY
    // Theo dõi trạng thái đăng nhập
    auth.onAuthStateChanged(async (user) => {
        if (user) {
            currentUser = user;

            // 1. Tải toàn bộ dữ liệu và CHỜ cho đến khi hoàn tất.
            await loadAllData();

            // 2. SAU KHI DỮ LIỆU ĐÃ CÓ, mới hiển thị ứng dụng và gọi hàm init để vẽ.
            authContainer.style.display = 'none';
            appShell.style.display = 'flex';

            if (!document.getElementById('logoutBtn')) {
                const settingsTab = document.getElementById('tabCaiDat');
                const logoutBtn = document.createElement('button');
                logoutBtn.id = 'logoutBtn';
                logoutBtn.textContent = 'Đăng Xuất';
                logoutBtn.className = 'btn btn-danger btn-full-width';
                logoutBtn.style.marginTop = '20px';
                logoutBtn.onclick = () => auth.signOut();
                settingsTab.appendChild(logoutBtn);
            }

            // 3. Gọi init để vẽ giao diện với dữ liệu đã có sẵn.
            init();
        ...
        try {
            if (currentUser && navigator.onLine) {
                ...
                if (userDoc.exists) {
                    ...
                    window.scheduleData = firebaseData.scheduleData || {};
                    window.timesheetEntries = firebaseData.timesheetEntries || {};
                    // Gán dữ liệu vào biến toàn cục ngay lập tức

                    // Tải các cài đặt khác
                    const settings = firebaseData.settings || {};
                    firstDayIsMonday = settings.firstDayIsMonday === true;
                    window.baseWorkHours = settings.baseWorkHours || 8;
                    window.startWorkDay = settings.startWorkDay || 1;
                    displayMode = settings.displayMode || 'fullMonth';
                    currentCurrency = settings.currency || 'VND';
                    applyTheme(settings.theme || 'default');
                } else {
                    window.timesheetEntries = JSON.parse(localStorage.getItem(LOCAL_STORAGE_KEYS.timesheet)) || {};
                }
            } else {
                console.log("Offline hoặc không có user. Tải từ local.");
    ...
    const persistAllData = async () => {
        // Luôn lưu vào localStorage để dùng khi offline
        try {
            ...
            window.showToast("Lỗi không thể lưu dữ liệu offline!");
        }

        // Nếu có người dùng và đang online, đồng bộ lên Firebase
        if (currentUser && navigator.onLine) {
            try {
                ...
                await userDocRef.set({
                    ...
                    settings: {
                        ...
                        displayMode,
                        theme: localStorage.getItem(LOCAL_STORAGE_KEYS.theme) || 'default',
                        currency: currentCurrency
                    },
                    lastUpdated: firebase.firestore.FieldValue.serverTimestamp()
                }, { merge: true }); // Dùng merge:true để không ghi đè dữ liệu lương

                // Bạn có thể thêm một thông báo ở đây để biết đã đồng bộ thành công
                // console.log("Data successfully synced to Firebase!");

            } catch (error) {
                console.error("Error persisting data to Firebase:", error);
                window.showToast("Không thể đồng bộ dữ liệu. Kiểm tra lại quy tắc bảo mật.");
            }
        }
    };
...
signupBtn.addEventListener('click', () => {
    ...
    signupBtn.textContent = 'Xác Nhận Đăng Ký';
...
const handleSignUp = () => {
    ...
    if (name.length < 2) {
        authError.textContent = 'Vui lòng nhập tên của bạn.';
        return;
    }
...
// Gắn sự kiện cho nút đăng ký/xác nhận
signupBtn.addEventListener('click', () => {
    if (authTitle.textContent === 'Đăng Ký') {
        // Chuyển sang giao diện điền tên
        authTitle.textContent = 'Xác Nhận Tên';
        loginBtn.style.display = 'none';
        signupBtn.textContent = 'Hoàn Tất';
    } else {
        // Thực hiện đăng ký
        handleSignUp();
    }
...
forgotPasswordLink.addEventListener('click', (e) => {
    ...
    const email = emailInput.value;
    if (!email) {
        authError.textContent = 'Vui lòng nhập email để reset mật khẩu.';
        return;
    }
    auth.sendPasswordResetEmail(email)
        .then(() => {
            window.showToast('Link reset mật khẩu đã được gửi tới email của bạn!');
        })
        .catch((error) => {
            authError.textContent = error.message;
        });
