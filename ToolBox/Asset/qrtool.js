...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function drawCustomQr(data, shape, logoSource) {
        try {
            ...
            for (let row = 0; row < moduleCount; row++) {
                for (let col = 0; col < moduleCount; col++) {
                    if (qr.isDark(row, col)) {
                        ...
                        if (shape === 'dots') {
                            ctx.beginPath();
                            ctx.arc(x + cellSize / 2, y + cellSize / 2, cellSize / 2.3, 0, 2 * Math.PI);
                            ctx.fill();
                        } else {
            ...
            const finalizeDrawing = (logoImg) => {
                if (logoImg) {
                    const logoSize = logicalSize * 0.25;
                    const logoX = (logicalSize - logoSize) / 2;
                    const logoY = (logicalSize - logoSize) / 2;
                    const bgSize = logoSize + cellSize;
                    const bgX = (logicalSize - bgSize) / 2;
                    const bgY = (logicalSize - bgSize) / 2;
            ...
            console.error(e);
            errorMessage.textContent = 'Đã xảy ra lỗi. Dữ liệu có thể quá dài hoặc không hợp lệ.';
            qrCodeResult.classList.remove('hidden');
